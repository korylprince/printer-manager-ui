  ...
  <v-card width="100%" :max-width="maxWidth">
    ...
    >
      <template
        ...
        v-if="updateRoute || showDelete || showUnassign"
      >
      ...
      <template v-slot:no-data> No {{ name }}s found </template>
...
export default {
    ...
    methods: {
        _updateRoute(item) {
            ...
            return {
                name: this.updateRoute.name,
                params: {...this.updateRoute.params, id: item.id},
            }
        },
    },
}
