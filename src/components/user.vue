...
export default {
    ...
    computed: {
        ...
        groups_loading() {
            return this.is_loading(api.read_user_groups)
        },
    },
    watch: {
        id(newID) {
    ...
    methods: {
        ...
        async do_read(id) {
            try {
                const [user, locations, groups] = await Promise.all([
                    this.api_action({action: api.read_user, params: [id]}),
                    this.api_action({action: api.read_user_locations, params: [id]}),
                    this.api_action({action: api.read_user_groups, params: [id]}),
                ])
                ...
                if (err.response !== null && err.response.status === 404) {
                    this.$back()
                    this.ADD_FEEDBACK("User not found")
                }
            }
        },
