<template>
  <v-app>
    ...
    <v-app-bar id="app-bar" color="primary" dark dense clipped-left app>
    ...
    <v-dialog :value="confirm_dialog" persistent max-width="320">
      <v-card v-if="confirm_dialog">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="accent" text @click="UPDATE_CONFIRMATION(null)"
            >No</v-btn
          >
          <v-btn
            color="primary"
            text
            @click="
              confirm_dialog.callback(...confirm_dialog.params);
    ...
    <v-snackbar :value="current_feedback != null" @input="clear_feedback">
      {{ current_feedback }}
      <v-btn color="primary" text @click="clear_feedback"> Close </v-btn>
    </v-snackbar>
  </v-app>
</template>
