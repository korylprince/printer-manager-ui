    ...
    <v-col>
        <v-row align-content="center" justify="center">
            ...
            <v-card width="100%" max-width="600px" v-else>
                ...
                <validation-observer ref="form" v-slot="{pristine}">
                    <form novalidate @submit.prevent="id ? do_update(building_id, id, building.id, name) : do_create(building.id, name)">
                        <v-card-text>
                            <validation-provider name="building" rules="required" mode="passive" v-slot="{errors}">
                                <v-autocomplete
                                    label="Building"
                                    ...
                                    <template v-slot:append-outer v-if="building">
                                        <v-tooltip bottom>
                                            <template v-slot:activator="{on}">
                                                <v-btn text icon v-on="on" :to="{name: 'update-building', params: {id: building.id}}" exact>
        ...
        <v-row v-if="id" align-content="center" justify="center" style="margin-top: 20px">
            <app-list name="Printer"
                      ...
                      :items="printers"
                      sort-by="search_name"
                      :create-route="{name: 'create-location-printer', params: {building_id, location_id: id}}"
                      :update-route="{name: 'update-printer', params: {building_id, location_id: id}}"
                      show-delete @delete="do_delete_printer(building_id, id, $event.id)"
        ...
        <v-row v-if="id" align-content="center" justify="center" style="margin-top: 20px">
            <app-list name="User"
                      ...
                      show-unassign @unassign="do_unassign_user(building_id, id, $event.id)"
        ...
        <v-row v-if="id" align-content="center" justify="center" style="margin-top: 20px">
            <app-list name="Group"
                      ...
                      show-unassign @unassign="do_unassign_group(building_id, id, $event.id)"
                      :loading="groups_loading">
            </app-list>
        </v-row>

        <app-assign-dialog ref="assign_user"
                           name="User"
                           attr="search_name"
        ...
        <app-assign-dialog ref="assign_group"
                           name="Group"
                           attr="search_name"
...
export default {
    ...
    data() {
        return {
            ...
            name: "",
            printer_headers: [{text: "Hostname", value: "search_name"}, {text: "Model", value: "model_search_name"}, {text: "Actions", value: "actions", sortable: false, align: "end"}],
            local_printers: [],
    ...
    computed: {
        ...mapGetters(["is_loading", "building_cache", "building_cache_map", "printer_cache_map", "user_cache", "user_cache_map", "group_cache", "group_cache_map"]),
        _loading() {
            return this.is_loading(api.create_location, api.read_location, api.update_location, api.delete_location)
        },
        _loading_skel() {
        ...
        printers_loading() {
            return this.is_loading(api.query_building_location_printers, api.delete_printer)
        },
        users() {
            const users = []
        ...
        users_loading() {
            return this.is_loading(api.read_building_location_users, api.assign_building_location_user, api.unassign_building_location_user)
        },
        groups() {
            const groups = []
        ...
        groups_loading() {
            return this.is_loading(api.read_building_location_groups, api.assign_building_location_group, api.unassign_building_location_group)
        },
    },
    watch: {
        id(newID) {
    ...
    methods: {
        ...
        async do_create(building_id, name, edit=false) {
            ...
            try {
                const location = await this.api_action({action: api.create_location, params: [building_id, name]})
                this.update_cache([CacheTypes.Location])
                if (edit) {
                    this.$router.replace({name: "update-location", params: {building_id, id: location.id}})
                } else {
                    this.clean = location
                    this.$nextTick(() => {
                        if (!(this.building_id)) {
            ...
            if (id != null) {
                try {
                    const [location, printers, users, groups] = await Promise.all([
                        this.api_action({action: api.read_location, params: [building_id, id]}),
                        this.api_action({action: api.query_building_location_printers, params: [building_id, id]}),
                        this.api_action({action: api.read_building_location_users, params: [building_id, id]}),
                        this.api_action({action: api.read_building_location_groups, params: [building_id, id]}),
                    ])
                    this.clean = location
                    this.$nextTick(() => {
        ...
        async do_update(building_id, id, building_id_new, name) {
            ...
            try {
                const location = await this.api_action({action: api.update_location, params: [building_id, id, building_id_new, name]})
                this.update_cache([CacheTypes.Location])
                if (building_id !== building_id_new) {
                    this.$router.replace({name: "update-location", params: {building_id: building_id_new, id}})
                }
                this.clean = location
                this.edit = false
                this.$nextTick(() => {
        ...
        async do_delete_printer_callback(building_id, location_id, id) {
            try {
                ...
                this.local_printers = await this.api_action({action: api.query_building_location_printers, params: [building_id, location_id]})
        ...
        async do_assign_user(building_id, location_id, user_id) {
            ...
            this.local_users = await this.api_action({action: api.read_building_location_users, params: [building_id, location_id]})
        ...
        async do_assign_group(building_id, location_id, group_id) {
            ...
            this.local_groups = await this.api_action({action: api.read_building_location_groups, params: [building_id, location_id]})
