  ...
  <v-col>
      ...
      <v-card width="100%" max-width="600px" v-else>
        ...
        <validation-observer ref="form" v-slot="{ pristine }">
          <form
            ...
            @submit.prevent="
              id
                ? do_update(manufacturer_id, id, manufacturer.id, name, driver)
                : do_create(manufacturer.id, name, driver)
            "
          >
            <v-card-text>
              <validation-provider
    ...
    >
      <app-list
        ...
        sort-by="search_name"
        :create-route="{
          name: 'create-model-printer',
          params: { manufacturer_id, model_id: id },
        }"
        :update-route="
          (printer) => ({
            name: 'update-printer',
        ...
        show-delete
        @delete="
          do_delete_printer($event.building_id, $event.location_id, $event.id)
        "
...
export default {
    ...
    data() {
        return {
            ...
            printer_headers: [
                ...
                {text: "Location", value: "location_search_name"},
    ...
    methods: {
        ...
        async do_create(manufacturer_id, name, driver, edit = false) {
            ...
            try {
                const model = await this.api_action({
                    action: api.create_model,
                    params: [manufacturer_id, name, driver],
                })
                this.update_cache([CacheTypes.Model])
                if (edit) {
                    this.$router.replace({
                        name: "update-model",
                        params: {manufacturer_id, id: model.id},
                    })
                } else {
                    this.clean = model
                    this.$nextTick(() => {
            ...
            if (id != null) {
                try {
                    const [model, printers] = await Promise.all([
                        this.api_action({
                            action: api.read_model,
                            params: [manufacturer_id, id],
                        }),
                        this.api_action({
                            action: api.query_printers,
                            params: [{model_id: id}],
                        }),
                    ])
                    this.clean = model
        ...
        async do_update(manufacturer_id, id, manufacturer_id_new, name, driver) {
            ...
            try {
                const model = await this.api_action({
                    action: api.update_model,
                    params: [manufacturer_id, id, manufacturer_id_new, name, driver],
                })
                this.update_cache([CacheTypes.Model])
                if (manufacturer_id !== manufacturer_id_new) {
                    this.$router.replace({
                        name: "update-model",
                        params: {manufacturer_id: manufacturer_id_new, id},
                    })
                }
                this.clean = model
                this.edit = false
